...
def resolve_codesigning(arguments, base_path, build_configuration, provisioning_profiles_path, additional_codesigning_output_path) -> ResolvedCodesigningData:
    ...
    if additional_codesigning_output_path is not None:
        profile_source.copy_profiles_to_destination(
            destination_path=additional_codesigning_output_path + '/profiles'
        )
        profile_source.copy_certificates_to_destination(
            destination_path=additional_codesigning_output_path + '/certs'
        )
...
def resolve_configuration(base_path, bazel_command_line: BazelCommandLine, arguments, additional_codesigning_output_path):
    ...
    # 100GRAMM UNSIGNED BUILD
    #
    # For an unsigned build there may be no valid aps-environment
    # in the fake provisioning profiles. Keep the Push Notifications
    # entitlement by providing a development aps-environment value.
...
def resolve_watch_provisioning_profile(arguments, base_path):
    ...
    """
    is_distribution_codesigning = arguments.gitCodesigningType in (
        'appstore',
        'adhoc',
        'enterprise'
    )
    ...
    if explicit_profile is not None:
        if not os.path.exists(explicit_profile):
            raise Exception(
                '--watchProvisioningProfile was set to a path that does not exist: {}'.format(
                    explicit_profile
                )
            )
        return explicit_profile

    resolved_watch_profile = os.path.join(
        base_path,
        'build-input/configuration-repository/provisioning/WatchApp.mobileprovision'
    )
    ...
    if is_distribution_codesigning:
        raise Exception(
            '--embedWatchApp is set for a distribution build '
            '(--gitCodesigningType={ct}), but no watchkitapp '
            'provisioning profile resolved (looked for {p}).\n'
            'The {ct} codesigning material does not contain a '
            '`.watchkitapp` profile, so the embedded watch app '
            'would be UNSIGNED — the host app is not re-signed over it, '
            'so it ships unsigned and is silently rejected when installing '
            'on a watch.\n'
            'Fix: fetch the latest codesigning material so the watchkitapp '
            'profile is present — drop --gitCodesigningUseCurrent '
            '(it skips the fetch), or run '
            '`git -C build-input/configuration-repository-workdir/encrypted pull` '
            '— or create/register the {ct} watchkitapp provisioning profile, '
            'or pass an explicit --watchProvisioningProfile.'.format(
                ct=arguments.gitCodesigningType,
                p=resolved_watch_profile
            )
        )

    print(
        'TelegramBuild: warning: --embedWatchApp is set but no watch provisioning '
        'profile was found (pass --watchProvisioningProfile, or use codesigning '
        'material that includes the watchkitapp profile; looked for {}). '
        'The embedded watch app will be UNSIGNED and rejected by the App Store.'
        .format(resolved_watch_profile)
    )
...
def build(bazel, arguments):
    ...
    if arguments.embedWatchApp:
        if arguments.configuration in ('debug_arm64', 'release_arm64'):
            if arguments.watchApiId is None or arguments.watchApiHash is None:
                raise Exception(
                    '--embedWatchApp requires --watchApiId and --watchApiHash '
                    '(the embedded watch app build needs API credentials).'
                )

            watch_provisioning_profile = resolve_watch_provisioning_profile(
                arguments=arguments,
                base_path=os.getcwd()
            )
            ...
            print(
                'TelegramBuild: warning: --embedWatchApp requires a device '
                'configuration (debug_arm64 or release_arm64); ignored for '
                'simulator builds.'
            )
    ...
    # 100GRAMM UNSIGNED BUILD
    # Enable the Bazel flag that disables provisioning-profile based signing.
    if arguments.disableProvisioningProfiles:
        bazel_command_line.set_disable_provisioning_profiles()

    bazel_command_line.set_split_swiftmodules(
        arguments.enableParallelSwiftmoduleGeneration
    )
    ...
    if arguments.outputBuildArtifactsPath is not None:
        ...
        ipa_paths = glob.glob(
            '{}/Telegram.ipa'.format(built_ipa_path_prefix)
        )

        if len(ipa_paths) == 0:
            print(
                'Could not find the IPA at '
                '{}/Telegram.ipa'.format(built_ipa_path_prefix)
            )
            sys.exit(1)
        elif len(ipa_paths) > 1:
            print(
                'Multiple matching IPA files found: {}'.format(ipa_paths)
            )
            sys.exit(1)

        shutil.copyfile(
            ipa_paths[0],
            artifacts_path + '/Telegram.ipa'
        )
        ...
        for dsym_path in dsym_paths:
            ...
            shutil.copytree(
                dsym_path,
                artifacts_path + '/DSYMs/{}'.format(file_name)
            )
...
if __name__ == '__main__':
    ...
    generateProjectParser = subparsers.add_parser(
        'generateProject',
    ...
    buildParser.add_argument(
        ...
        default=False,
        help='''
            Build without provisioning profiles.
            '''
    )
    ...
    remote_build_parser = subparsers.add_parser(
        'remote-build',
        help='Build the app using a remote environment.'
    )
    ...
    vm_build_parser = subparsers.add_parser(
        'vm-build',
        help='Build the app using a VM.'
    )
    ...
    generate_profiles_build_parser = subparsers.add_parser(
        'generate-verification-profiles',
        help='Generate provisioning profiles that can be used to build a veritication IPA.'
    )
    ...
    remote_upload_testflight_parser = subparsers.add_parser(
    ...
    remote_ipadiff_parser = subparsers.add_parser(
        'remote-ipa-diff',
        help='Execute ipa-diff using a remote environment.'
    )
    ...
    query_parser = subparsers.add_parser(
        'query',
        help='Run arbitrary bazel queries'
    )
    ...
    spm_parser = subparsers.add_parser(
        'spm',
        help='Generate SPM package (outputs bazel-bin/Telegram/spm_build_root_modules.json)'
    )
    ...
    if args.bazel is None:
        bazel_path = locate_bazel(
            base_path=os.getcwd(),
            cache_host_or_path=args.cacheHost,
    ...
    try:
        if args.commandName == 'clean':
            ...
            versions = BuildEnvironmentVersions(
                base_path=os.getcwd()
            )
            ...
            shutil.copyfile(
                ...
                remote_input_path + '/configuration.json'
            )
            ...
            if args.embedWatchApp:
                ...
                if args.watchProvisioningProfile is not None:
                    shutil.copyfile(
                        args.watchProvisioningProfile,
                        remote_input_path + '/watch_provisioning_profile.mobileprovision'
                    )

                    watch_provisioning_profile_remote_path = (
                        '$HOME/telegram-build-input/watch_provisioning_profile.mobileprovision'
                    )
            ...
            GenerateProfiles.generate_provisioning_profiles(
                source_path=remote_input_path + '/profiles',
